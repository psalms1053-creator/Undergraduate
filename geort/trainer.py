        ...
        for epoch in range(n_epoch):
            for batch_idx, batch in enumerate(point_dataloader):
                ...
                if batch_idx % 50 == 0:
                    ...
                    '''
                    # [1] 먼저 실제 반영되는 값(Weighted Loss)을 계산합니다.
                    # 보통 config나 args에서 가져옵니다. 예: args.w_direction
                    val_dir = direction_loss.item()
                    val_cham = chamfer_loss.item() * w_chamfer
                    val_curv = curvature_loss.item() * w_curvature
                    val_pinch = pinch_loss.item() * w_pinch
                    val_coll = collision_loss.item() * w_collision
                    
                    
                    # [2] 로그 출력 수정
                    # if batch_idx % 50 == 0:
                    print("-" * 80)
                    print(f"Epoch {epoch} | Batch {batch_idx} | Weighted Contribution Check")
                    print(f" 1. Direction : {format_loss(direction_loss.item())} (raw) = {val_dir:.4f}")
                    print(f" 2. Chamfer   : {format_loss(chamfer_loss.item())} (raw) x {w_chamfer} (w) = {format_loss(val_cham)}    |"
                          f" 3. Curvature : {format_loss(curvature_loss.item())} (raw) x {w_curvature} (w) = {format_loss(val_curv)}")
